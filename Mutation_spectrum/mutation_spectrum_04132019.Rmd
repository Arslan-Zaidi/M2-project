...
Get summary statistics presented in the paper.

```{r sum_stats, echo=FALSE}

#summary stats

#number of independent mutation events
print(
  paste("Number of independent mutation events:",length(unique(hq$fam_het_id))))
...
print(paste("Number of unique sites:",
            length(uniq_sites)))
...
# print("Number of families carrying a heteroplasmy at each site")
# uniq_sites_fam

#how many sites are present in one family only

print(paste("Number of sites present in one family:",
            uniq_sites_fam%>%
  filter(nfams==1)%>%
  nrow()
))
...
print(paste("Number of sites present in 2 families:",
            uniq_sites_fam%>%
  filter(nfams==2)%>%
  nrow()
))
...
#how many sites are present in >3 families
print(paste("Number of sites present in >3 families:",
            uniq_sites_fam%>%
  filter(nfams>=3)%>%
  nrow()
))
...
```{r seq_context, cache=TRUE, warning=F, results='hide', message=F}
#load count file

count<-fread(here("../../M2_new/files/Analysis/variant_calling/trim.rd.counts/Allruns_08222018.trim.rd.counts"),
             header=F,
             sep="\t")

#^THIS IS A LOCAL PATH. to access the count file, comment out the above line and uncomment the one below >>
...
Generate sequence context for each heteroplasmy
...
Now, let's test whether there is enrichment of heteroplasmies in the Dloop. 

```{r}

dloop_hqn<-hq%>%
  distinct(fam_het_id,.keep_all=T)%>%
  mutate(region=case_when(gene%in%c("D-loop1","D-loop2")~"Dloop",
                          TRUE~"non-D"))%>%
  group_by(region)%>%
  summarize(n=length(unique(fam_het_id)))

#get total length of D-loop and that of non-D
dlength=genedb%>%
  filter(gene%in%c("D-loop1","D-loop2"))%>%
  mutate(length=end-start)%>%
  pull(length)%>%
  sum()

nondlength=16569-dlength

#calculate expected number of heteroplasmies given the lenght of the region
dloop_hqn$exp_n<-c(dlength,nondlength)/16569*sum(dloop_hqn$n)

dbtest=binom.test(x=73,n=(346),p=23.43/(346))
